...
class Simplex():
    ...
    def display_final_table_corrected(self):
        # Encontrar variáveis básicas e não-básicas
        basic_vars = self.find_basic_variables()

        # Ajustar rótulos das colunas para as variáveis de decisão e de folga
        decision_var_labels = [f'X{i+1}' for i in range(self.n_var)]
        slack_var_labels = [f'R{i+1}' for i in range(self.n_restricoes)]

        # Ajustar os rótulos das colunas com base no tableau final, considerando variáveis artificiais se houver
        col_labels = decision_var_labels + slack_var_labels
        col_labels = col_labels[:self.tableau.shape[1] - 1] + ['LD']

        # Ajustar os rótulos das linhas com as variáveis básicas encontradas
        row_labels = basic_vars + ['Z']

        # Imprimir a tabela final com os rótulos corretos
        print("Tabela Final:")
        print("    |", ' '.join(col_labels))
        for i, row in enumerate(self.tableau):
            print(f"{row_labels[i]:>3} |", ' '.join(f"{val:8.2f}" for val in row))

    def find_basic_variables(self):
        # Encontrar as variáveis básicas na solução final
        basic_vars = []
        for i in range(self.tableau.shape[0] - 1):  # Excluir a linha Z
            basic_var_index = np.where(self.tableau[i, :-1] == 1)[0]
            if len(basic_var_index) == 1 and np.all(self.tableau[i, basic_var_index[0]] == 1):
                if basic_var_index[0] < self.n_var:
                    basic_vars.append(f'X{basic_var_index[0]+1}')
                else:
                    basic_vars.append(f'R{basic_var_index[0]+1-self.n_var}')
            else:
                basic_vars.append('Artificial' or 'Redundant')
...
# Plotando a tabela final
teste.display_final_table_corrected()